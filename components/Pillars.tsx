    ...
    <section className="section">
      <div className="shell">
        ...
        <div className="mt-14 grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:gap-16">
          ...
          <div className="grid content-center gap-x-9 sm:grid-cols-2">
            {p.items.map((item, i) => (
              <article key={item.no} className={`reveal py-6 ${i === 4 ? "sm:col-span-2" : ""}`}>
                <span className="font-mono text-xs text-wine-700">{item.no}</span>
                <h3 className="mt-3 text-xl font-medium leading-tight tracking-[-0.02em] text-green-950">{item.title}</h3>
                <p className="mt-3 max-w-lg text-[0.9375rem] leading-[1.7] text-ink-2">{item.body}</p>
              </article>
            ))}
          </div>
        </div>
